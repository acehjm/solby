...
public class DateUtil {
    ...
    public static Date parseString(String dateStr, String dateFormat, String locale) {
        ...
        if (StringUtils.isNotBlank(dateFormat)) {
        ...
        else if (PatternUtil.matches(dateStr, "[a-zA-Z]")) {
            ...
            // 中文日期格式
            if (PatternUtil.matches(dateStr, "[年月日时分秒]"))
                dateStr = parseChinaDate(dateStr);
                // 含中文，但非标准的时间性中文
            else if (PatternUtil.hasChinese(dateStr)) {
                return null;
            }
            int size;
            ...
            if (hasBlank) {
                ...
                if (dateStr.indexOf(":") != -1) {
                    ...
                    splitCount = PatternUtil.matchCnt(dateStr, "\\/");
                    if (splitCount == 2) {
        ...
        DateFormat df = (StringUtils.isBlank(locale)) ? new SimpleDateFormat(realDF)
    ...
    public static String formatDate(Object dt, String fmt, String locale) {
        ...
        DateFormat df = StringUtils.isBlank(locale) ? new SimpleDateFormat(fmt)
    ...
    public static String parseChinaDate(String chinaDate, String fmt) {
        if (StringUtils.isBlank(chinaDate))
            return null;
        // 去除中文日期文字之间的空格
